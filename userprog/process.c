...
process_execute (const char *file_name) 
{
  ...
  /* Create a new thread to execute FILE_NAME. */
	fn_name= strtok_r (temp, " ", &save_ptr);
	
	struct file *f=filesys_open(fn_name);    //test if the file is right, exec missing
	if(f==NULL)
		return -1;
...
start_process (void *file_name_)
{
  ...
  success = load (argv[0], &if_.eip, &if_.esp);
	...
	//pass argument参数传递	
	//If load successed如果存储成功了的话
	if (success)
	{
		// Push argument into stack把参数压入堆栈
		n = argc-1;
		...
		//Word align字对齐，四位一对齐
		int offset = (int)if_.esp % 4;//先算出要多少个offset
		if (offset != 0)
		{
			if(offset<0)
...
//将参数传递到寄存器中,因为我们的地址是int类型的，而且地址是向下增长的，所以每次在内存中传地址的时候，我们的esp的指针必须每次向下移动4个字节，将argv中的内容根据argc的大小进行传递
		...
		//Push the argument address in stack into the stack参数地址入栈
		...
		*(void **)if_.esp = (char **) if_.esp + 1;
		...
		// Push the number of the arguments into stack把参数个数压栈
		if_.esp = if_.esp - 4;//栈顶指针向前移动出一个空位
		*(int *)if_.esp = argc;//把个数赋给*(int *)if_.esp

		// Push return address into stack.返回地址入栈
		if_.esp = if_.esp - 4;//栈顶指针向前移动出一个空位
...
{	
  ...
  return t->exit_status;//返回当前t的退出状态
}
